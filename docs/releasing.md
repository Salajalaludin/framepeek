...
For version `0.1.1`, the build produces:

```text
dist/0.1.1/framepeek-0.1.1.tar.gz
dist/0.1.1/framepeek-0.1.1-py3-none-any.whl
```
...
```bash
python -m pip install dist/0.1.1/framepeek-0.1.1-py3-none-any.whl
...
an absolute path. Confirm that package metadata reports version `0.1.1`:

```bash
python -c "from importlib.metadata import version; assert version('framepeek') == '0.1.1'"
```
...
1. Move `Unreleased` changes into a dated `0.1.1` changelog entry.
2. Merge that change through the normal pull-request workflow.
3. Tag the exact merge commit as `v0.1.1`.
4. Publish the verified files from `dist/0.1.1/`.
